...
## GET /your-things/{persona}  *(MT15 — owner-side dormant inventory)*
Every product the persona owns, valued live as idle resale stock — the **pull twin** of the demand-driven Idle-Asset Radar. A still-owned, well-kept unit is valued at a like-kept grade `B` and depreciated by age via `pricing.resale_value`; the next-month delta is `decay_per_month`. The life-stage curve (shared with `/life-stage`) supplies `stage_pct`/`stage_label`/`due_to_resell`. Highest dormant value first. `404` if the persona has no seeded order history. Pure deterministic Python — no LLM.
→ `200 {"persona": "rahul", "total_dormant_value": 12040, "item_count": 8, "due_count": 3, "things": [{"order_id": "...", "asin": "...", "item_id": "...", "title": "...", "category": "...", "thumb": "...", "purchase_date": "...", "price_paid": 3200, "months_owned": 0, "typical_life_months": 36, "stage_label": "useful life", "stage_pct": 0, "due_to_resell": false, "resale_value": 3249, "decay_per_month": 0, "resellable": true}, ...]}`. Only catalog-resolved items are `resellable` (they can enter the resell grade/price flow). Frontend: the buyer storefront's "Your things" tab; per-thing Resell CTA routes into the existing resell flow.

## GET /green-ledger/{persona}  *(MT15 — per-persona impact ledger)*
The global `/metrics` impact scoped to one persona's own products: CO₂ / landfill-kg / items diverted, derived from that persona's `ROUTED` passport events plus a small seeded baseline (mirrors `metrics.BASELINE`) so the strip reads non-zero before the live demo; reuses `metrics.LANDFILL_KG`. Stateless read, no LLM. Shown **subtly** (a small strip, not a hero panel) on both the buyer's Your Things hero and the seller dashboard.
