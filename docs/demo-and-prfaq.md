...
| 2:35–2:50 | **Architecture + metrics** | One diagram (multimodal vision → VRS → DynamoDB passport) + batch counter: "8 items → ₹4,830 recovered vs ₹0" + CO₂ ticker | "Serverless on AWS — multimodal vision for perception with Bedrock Nova as the AWS-native failover, auditable code for money. Amazon earns a take-rate on every rupee recovered: we only earn when value is saved." |
